...
```
#### Coleta de Dados: PIB dos Municípios (Conexão Direta IPv4)
Nesta etapa, conectamos diretamente à API do IBGE usando protocolo IPv4 forçado e uma URL pré-formatada para evitar erros de caracteres inválidos.

```{r coleta_pib_direta_v4, warning=FALSE, message=FALSE}
library(jsonlite)
library(dplyr)

# ==============================================================================
# 1. Obter PIB (Tabela 5938) - Ano 2021
# ==============================================================================
url_pib <- "https://apisidra.ibge.gov.br/values/t/5938/n3/all/v/37/p/2021?formato=json"
raw_pib <- fromJSON(url_pib)

# TRUQUE DE SEGURANÇA: Não renomeamos o cabeçalho. Usamos os códigos originais.
# D1C = Código da UF (ex: 11, 35)
# D1N = Nome da UF (ex: Rondônia, São Paulo)
# V   = Valor
...
# ==============================================================================
# 2. Obter População (Tabela 6579) - Ano 2021
# ==============================================================================
...
# 1. Baixar dados (PNAD - Rendimento Domiciliar per Capita - Último ano)
...
# 2. Tratamento Corrigido
# Mapeamos D3N como Ano (que é o padrão quando D2N é a descrição)
df_renda <- raw_pnad[-1, ] %>%
  as_tibble() %>%
  select(
...
# Mostra o ano para confirmar (deve aparecer 2023 ou 2024)
print(head(df_renda))
```

```{r}

# 1. Preparação da Tabela de Renda (Seleção segura)
df_renda_clean <- df_renda %>%
  select(COD_UF, Renda_Domiciliar, matches("Ano|Trimestre"))

# 2. Consolidação (Join e Cálculos)
