...
graf_sin <- ggplot(df_grafico, aes(x = reorder(Acao, -Sinalizacoes), y = Sinalizacoes)) +
...
df_fundo <- as.data.frame(simulacao_matrix[, 1:100]) %>% 
  mutate(Dia = 0:(nrow(simulacao_matrix) - 1)) %>%
  pivot_longer(cols = -Dia, names_to = "Simulacao", values_to = "Valor")

# --- 2. Gerar o Gráfico Interativo ---

fig <- plot_ly() %>%
  # Camada 1: As linhas de fundo (amostra das simulações)
  add_trace(data = df_fundo, x = ~Dia, y = ~Valor, split = ~Simulacao,
            type = 'scatter', mode = 'lines',
            line = list(color = 'rgba(0, 0, 255, 0.1)', width = 0.5), # Azul bem transparente
            showlegend = FALSE, hoverinfo = "none") %>% # Sem tooltip no fundo para não poluir
  
  # Camada 2: A Média (Linha preta sólida)
...
saveRDS(fig, "app_para_publicar/montecarlo.rds")
saveRDS(p_graf_sin, "app_para_publicar/rand_forest.rds")
